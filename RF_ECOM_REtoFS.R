...
REGION <- 'EMEA'
FILE <- paste0('data/transformed/', REGION, '_ECOM_HLI.rds')
...
#                   c('APJ'         , 'AMS'     , 'EMEA'))
#sqlECOM <- sprintf("SELECT * FROM [dbo].[eOrders_HLI_CLEAN_OSS_v] WHERE [Profit Center L0 Name] = '%s'", L0NAME[REGION])
...
dfTEST <- data.frame(
  ...
  #RF_BASE_ALL_SEG80  = ceiling(prBASEALLTest[[targetList[2]]] + prBASEALLTest80[[targetList[3]]]) + PaddingFactor,
  ...
  #RF_BASE_SCG_SEG80  = ceiling(prBASESCGTest[[targetList[2]]] + prBASESCGTest80[[targetList[3]]]) + PaddingFactor,
  ...
  #RF_ENG_ALL_SEG80   = ceiling(prENGALLTest[[targetList[2]]] + prENGALLTest80[[targetList[3]]]) + PaddingFactor,
  ...
  #RF_ENG_SCG_SEG80   = ceiling(prENGSCGTest[[targetList[2]]] + prENGSCGTest80[[targetList[3]]]) + PaddingFactor,
...
dfTESTDG <- dfTEST %>% group_by(DeliveryGroupID) %>%
  summarise(MaxGroupList       = max(as.character(GroupList)),
            ...
            RF_BASE_ALL_UNSEG  = max(RF_BASE_ALL_UNSEG),
            RF_BASE_ALL_SEG    = max(RF_BASE_ALL_SEG),
            #RF_BASE_ALL_SEG80  = max(RF_BASE_ALL_SEG80),
            RF_BASE_ALL_SEG85  = max(RF_BASE_ALL_SEG85),
            RF_BASE_SCG_UNSEG  = max(RF_BASE_SCG_UNSEG),
            RF_BASE_SCG_SEG    = max(RF_BASE_SCG_SEG),
            #RF_BASE_SCG_SEG80  = max(RF_BASE_SCG_SEG80),
            RF_BASE_SCG_SEG85  = max(RF_BASE_SCG_SEG85),
            RF_ENG_ALL_UNSEG   = max(RF_ENG_ALL_UNSEG),
            RF_ENG_ALL_SEG     = max(RF_ENG_ALL_SEG),
            #RF_ENG_ALL_SEG80   = max(RF_ENG_ALL_SEG80),
