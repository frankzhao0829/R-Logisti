...
if (RFPKG == 'quantregForest') {
  baseCatInput <- c(baseCatInput, 'PL_HPS', 'PL_HPSD', 'PL_HPN', 'FAG_FA', 'FAG_HS', 'FAG_P1')
} else {
  baseCatInput <- c(baseCatInput, 'ProdLineID', 'FocusAccountGroup')
}
...
out <- ModelEvaluate(predicted = dfTRAIN %>%
                     ...
                     dfTRAIN$Target,
                     RFDataSet[train, ]$SupplierComplexityGroup,
                     QTarget)
...
out <- ModelEvaluate(predicted = dfTEST %>%
                       select(contains('SEG'), ATP, ALLQTargetLT, SCGQTargetLT),
                     dfTEST$Target,
                     RFDataSet[test, ]$SupplierComplexityGroup,
                     QTarget)
